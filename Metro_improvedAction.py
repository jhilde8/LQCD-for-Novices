# -*- coding: utf-8 -*-
"""
Created on Tue Feb 14 13:23:49 2023
...
def S(i,x): # harm. osc. S
    ...
    i_m1 = (i-1)%len(x)
    i_m2 = (i-2)%len(x)
    return a**2*(x[i]**2)/2 + (m/(24*a)) * x[i] * (x[i_m2]-16*x[i_m1]+15*x[i]-16*x[i_p1]+x[i_p2])
...
def S(j,x):
    ...
    return a**2*(x[j]**2/2) + (1/(24*a))*(m*x[j])*((-16)*(x[jp] + x[jm])+(x[jpp] + x[jmm])+(15)*x[j])
...
ax.set_title('Metropolis for QHO - Improved action - kinetic')
ax.legend()
